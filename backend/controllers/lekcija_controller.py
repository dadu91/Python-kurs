...
def vrati_lekciju_po_id(lekcija_id: int, db: Session = Depends(get_db)):
...
def dodaj_lekciju(lekcija: LekcijaCreate, db: Session = Depends(get_db)):
    return lekcija_service.create_lekcija(db, lekcija)


@router.put("/{lekcija_id}", response_model=LekcijaOut)
def izmijeni_lekciju(
    lekcija_id: int,
    lekcija_update: LekcijaUpdate,
...
def obrisi_lekciju(lekcija_id: int, db: Session = Depends(get_db)):
    lekcija = lekcija_service.get_lekcija_id(db, lekcija_id)
