...
def kreiraj_gresku(greska: GreskaCreate, db: Session = Depends(get_db), current_user: Korisnik = Depends(get_current_user)):
    return greska_service.create_greska(db, greska)

@router.put("/{greska_id}", response_model=GreskaOut)
def izmijeni_gresku(greska_id: int, greska: GreskaUpdate, db: Session = Depends(get_db), current_user: Korisnik = Depends(get_current_user)):
    return greska_service.update_greska(db, greska_id, greska)

@router.delete("/{greska_id}", status_code=204)
def izbrisi_gresku(greska_id: int, db: Session = Depends(get_db), current_user: Korisnik = Depends(get_current_user)):
