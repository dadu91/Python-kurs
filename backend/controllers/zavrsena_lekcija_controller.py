...
def citaj_sve_zavrsene_lekcije(db: Session = Depends(get_db)):
    return zavrsena_lekcija_service.get_zavrsena_lekcija_all(db)

@router.get("/po-id/{id}", response_model=ZavrsenaLekcijaOut)
def citaj_zavrsenu_lekciju_preko_id(id: int, db: Session = Depends(get_db)):
...
def citaj_zavrsenu_lekciju_preko_korisnik_id(korisnik_id:int, db: Session = Depends(get_db)):
...
def citaj_zavrsenu_lekciju_preko_lekcija_id(lekcija_id:int, db: Session = Depends(get_db)):
...
def kreiraj_zavrsenu_lekciju(zavrsena_lekcija: ZavrsenaLekcijaCreate, db: Session = Depends(get_db)):
