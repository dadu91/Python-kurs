...
from schemas.progres_schema import ProgresOut, ProgresCreate
...
@router.post("/", response_model=ProgresOut)
def kreiraj_progres(data: ProgresCreate, db: Session = Depends(get_db), current_user: Korisnik = Depends(get_current_user)):
    return progres_service.create_progres(db, data)